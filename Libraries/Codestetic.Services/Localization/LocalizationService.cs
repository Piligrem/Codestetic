...
namespace Codestetic.Web.Services.Localization
{
    ...
    {
        #region Constants
        private const string LOCALESTRINGRESOURCES_ALL_KEY = "specter.lsr.all-{0}";
        private const string LOCALESTRINGRESOURCES_PATTERN_KEY = "specter.lsr.";
        #endregion Constants
        ...
        /// <returns>Locale string resource</returns>
        public virtual LocaleStringResource GetLocaleStringResourceByName(string resourceName, long languageId, bool logIfNotFound = true)
        {
        ...
        public virtual void InsertLocaleStringResource(LocaleStringResource localeStringResource)
        {
            ...
            //// cache
            var holder = this.GetResourceValues(localeStringResource.LanguageId) as ConcurrentDictionary<string, Tuple<long, string>>;
            holder.TryAdd(
                localeStringResource.ResourceName,
                new Tuple<long, string>(localeStringResource.Id, localeStringResource.ResourceValue));
        ...
        public virtual void UpdateLocaleStringResource(LocaleStringResource localeStringResource)
        {
            ...
            var holder2 = holder as ConcurrentDictionary<string, Tuple<long, string>>;
            holder2.TryAdd(
                localeStringResource.ResourceName,
                new Tuple<long, string>(localeStringResource.Id, localeStringResource.ResourceValue));
        ...
        public virtual IDictionary<string, Tuple<long, string>> GetResourceValues(long languageId, bool forceAll = false)
        {
            Action<ConcurrentDictionary<string, Tuple<long, string>>> loadAllAction = (d) =>
            {
                ...
                foreach (var locale in locales)
                {
                    ...
                    d.AddOrUpdate(
                        resourceName,
                        (k) => new Tuple<long, string>(locale.Id, locale.ResourceValue),
                        (k, v) => { d[k] = v; return v; });
                    //}
                }

                // perf: add a dummy item indicating that data is fully loaded
                d.TryAdd("!!___EOF___!!", new Tuple<long, string>(0, string.Empty));
            };

            string cacheKey = string.Format(LOCALESTRINGRESOURCES_ALL_KEY, languageId);
            var dict = _cacheManager.Get(cacheKey, () =>
            {
                var result = new ConcurrentDictionary<string, Tuple<long, string>>(8, 2000, StringComparer.CurrentCultureIgnoreCase);
        ...
        public virtual string GetResource(string resourceKey, long languageId = 0, bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
        {
            if (languageId <= 0)
            {
                if (_workContext.WorkingLanguage == null)
            ...
            var holder = this.GetResourceValues(languageId) as ConcurrentDictionary<string, Tuple<long, string>>;

            Tuple<long, string> lsr = null;
            try
            {
                lsr = holder.GetOrAdd(resourceKey, (x) =>
                {
                    ...
                    if (res != null)
                        return new Tuple<long, string>(res.Id, res.ResourceValue);
                    return null;
                });
            }
            catch { }
        ...
        private class LocaleStringResourceParent : LocaleStringResource
        {
            public LocaleStringResourceParent(XmlNode localStringResource, string nameSpace = "")
            {
                ...
                {
                    throw new SpecterException("All language resources must have an attribute Name=\"Value\".");
                }
                var resName = resNameAttribute.Value.Trim();
                if (string.IsNullOrEmpty(resName))
                {
                    throw new SpecterException("All languages resource attributes 'Name' must have a value.'");
                }
