...
namespace Codestetic.Web.Services.Directory
{
    ...
    {
        #region Constants
        private const string CURRENCIES_ALL_KEY = "specter.currency.all-{0}";
        private const string CURRENCIES_PATTERN_KEY = "specter.currency.";
        private const string CURRENCIES_BY_ID_KEY = "specter.currency.id-{0}";
        #endregion
        ...
        public virtual decimal ConvertToPrimaryExchangeRateCurrency(decimal amount, Currency sourceCurrencyCode)
        {
            ...
            if (result != decimal.Zero && sourceCurrencyCode.Id != primaryExchangeRateCurrency.Id)
            {
                ...
                if (exchangeRate == decimal.Zero)
                    throw new SpecterException(string.Format("Exchange rate not found for currency [{0}]", sourceCurrencyCode.Name));
        ...
        public virtual decimal ConvertFromPrimaryExchangeRateCurrency(decimal amount, Currency targetCurrencyCode)
        {
            ...
            if (result != decimal.Zero && targetCurrencyCode.Id != primaryExchangeRateCurrency.Id)
            {
                ...
                if (exchangeRate == decimal.Zero)
                    throw new SpecterException(string.Format("Exchange rate not found for currency [{0}]", targetCurrencyCode.Name));
                result = result * exchangeRate;
            }
            return result;
        }
