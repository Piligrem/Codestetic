...
namespace Codestetic.Web.Services.Logging
{
    public partial class DefaultLogger : ILogger
    {
            ...
            string message, LogLevel? logLevel, int pageIndex, int pageSize, int minFrequency)
        {
            ...
            if (fromUtc.HasValue)
            {
                fromUtc = fromUtc.Value.Date;
                query = query.Where(l => fromUtc.Value <= l.CreatedOnUtc);
            }
            if (toUtc.HasValue)
            {
                toUtc = toUtc.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59);
                query = query.Where(l => toUtc.Value >= l.CreatedOnUtc);
            }
            if (logLevel.HasValue && logLevel != 0)
            {
        ...
        public virtual IEnumerable<Log> GetAllLogs()
        {
            var query = _logRepository.Table;
            query = query.OrderByDescending(l => l.CreatedOnUtc);
