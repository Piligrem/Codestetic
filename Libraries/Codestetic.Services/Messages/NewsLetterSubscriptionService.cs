...
namespace Codestetic.Web.Services.Messages
{
    public class NewsLetterSubscriptionService : INewsLetterSubscriptionService
    {
            ...
            using (var scope = new DbContextScope(ctx: _context, autoDetectChanges: false, proxyCreation: false, validateOnSave: false, autoCommit: false))
            {
                using (var reader = new StreamReader(stream))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        {
                            throw new SpecterException("Wrong file format (expected comma separated entries 'Email' and optionally 'IsActive')");
                        }
        ...
        public IPagedList<NewsLetterSubscription> GetAllNewsLetterSubscriptions(string email, int pageIndex, int pageSize, bool showHidden = false)
        {
            throw new NotImplementedException();
        }
    }
