    ...
    public partial class UserRegistrationService : IUserRegistrationService
    {
        ...
        public virtual UserRegistrationResult RegisterUser(UserRegistrationRequest request)
        {
            ...
            if (registeredRole == null)
                throw new SpecterException("'Registered' role could not be loaded");
        ...
        public virtual void SetEmail(User user, string newEmail)
        {
            ...
            if (!newEmail.IsEmail())
                //throw new SpecterException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));
                throw new SpecterException("New email is not valid");

            if (newEmail.Length > 100)
                //throw new SpecterException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));
                throw new SpecterException("E-mail address is too long");

            var user2 = _userService.GetUserByEmail(newEmail);
            if (user2 != null && user.Id != user2.Id)
                //throw new SpecterException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
                throw new SpecterException("The e-mail address is already in use");
        ...
        public virtual void SetUsername(User user, string newUsername)
        {
            ...
            if (!_userSettings.UsernamesEnabled)
                throw new SpecterException("Usernames are disabled");

            if (!_userSettings.AllowUsersToChangeUsernames)
                throw new SpecterException("Changing usernames is not allowed");

            newUsername = newUsername.Trim();

            if (newUsername.Length > 100)
                //throw new SpecterException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));
                throw new SpecterException("Username is too long");

            var user2 = _userService.GetUserByUsername(newUsername);
            if (user2 != null && user.Id != user2.Id)
                //throw new SpecterException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
                throw new SpecterException("The username is already in use");
