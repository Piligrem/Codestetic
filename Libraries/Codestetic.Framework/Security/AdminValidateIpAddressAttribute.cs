    ...
    public class AdminValidateIpAddressAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (!ok)
            {
                ...
                if (!thisPageUrl.StartsWith(string.Format("{0}admin/security/accessdenied", "http://"/*webHelper.GetSiteLocation()*/), StringComparison.InvariantCultureIgnoreCase))
                {
                    //redirect to 'Access denied' page
                    filterContext.Result = new RedirectResult(/*webHelper.GetSiteLocation()*/"http://" + "admin/security/accessdenied");
