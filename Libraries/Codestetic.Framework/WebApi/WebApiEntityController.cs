...
{
	public abstract class WebApiEntityController<TEntity, TService> : EntitySetController<TEntity, long>
		where TEntity : BaseEntity, new()
	{
		...
		protected override TEntity GetEntityByKey(long key)
		{
			if (!ModelState.IsValid)
				throw this.ExceptionInvalidModelState();
        ...
        protected internal virtual TEntity GetEntityByKeyNotNull(long key)
		{
			var entity = GetEntityByKey(key);
        ...
        protected internal virtual SingleResult<TEntity> GetSingleResult(long key)
		{
			if (!ModelState.IsValid)
				throw this.ExceptionInvalidModelState();
        ...
        public override HttpResponseMessage Put(long key, TEntity update)
		{
			return Request.CreateResponse(HttpStatusCode.OK, UpdateEntity(key, update));
		}

        protected override TEntity UpdateEntity(long key, TEntity update)
		{
			if (!ModelState.IsValid)
				throw this.ExceptionInvalidModelState();
		...
		public override HttpResponseMessage Patch(long key, Delta<TEntity> patch)
		{
			return Request.CreateResponse(HttpStatusCode.OK, PatchEntity(key, patch));
		}

        protected override TEntity PatchEntity(long key, Delta<TEntity> patch)
		{
			if (!ModelState.IsValid)
				throw this.ExceptionInvalidModelState();
