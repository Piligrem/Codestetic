...
namespace Specter.Web.Framework.WebApi
{
	public static class WebApiCaching
	{
				...
				if (key == WebApiUserCacheData.Key)
				{
					...
					if (cacheData != null)
					{
						...
						if (dataToStore.Count() > 0)
						{
							...
							{
								var dbContext = new SpecterObjectContext(DataSettings.Current.DataConnectionString);
		...
		public static List<WebApiUserCacheData> UserData()
		{
			return _get<List<WebApiUserCacheData>>(WebApiUserCacheData.Key, () =>
			{
				...
				var users = engine.Resolve<IRepository<User>>();

				var attributes = (
					from a in genericAttributes.Table
					join c in users.Table on a.EntityId equals c.Id
					where !c.Deleted && c.Active && a.KeyGroup == "User" && a.Key == WebApiUserCacheData.Key
					select new
					{
						a.Id,
						a.EntityId,
				...
				{
					if (!string.IsNullOrWhiteSpace(attribute.Value) && !data.Exists(x => x.UserId == attribute.EntityId))
					{
	...
	{
		public long GenericAttributeId { get; set; }
		...
		public bool IsValid
		{
			get
			{
				return GenericAttributeId != 0 && UserId != 0 && !string.IsNullOrWhiteSpace(PublicKey) && !string.IsNullOrWhiteSpace(SecretKey);
			}
		}
