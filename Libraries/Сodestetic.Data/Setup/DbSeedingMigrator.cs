	...
	public class DbSeedingMigrator<TContext> : DbMigrator where TContext : DbContext
	{
		...
		public int RunPendingMigrations(TContext context)
		{
			...
			foreach (var migrationId in pendingMigrations)
			{
				...
				coreSeeder = migration as IDataSeeder<SpecterObjectContext>;
				externalSeeder = null;
				...
				{
					if (context is SpecterObjectContext)
					{
						throw;
					}
			...
			SpecterObjectContext coreContext = null;
			if (coreSeeders.Any())
			{
				coreContext = isCoreMigration ? context as SpecterObjectContext : new SpecterObjectContext();
				RunSeeders<SpecterObjectContext>(coreSeeders, coreContext);
			}
